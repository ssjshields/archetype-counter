...
# Adds ability to move Image on from similar to normal window movement
Add-Type  @'
using System;
using System.ComponentModel;
using System.Runtime.InteropServices;
...
$Script:ArchetypeImage.Add_MouseDown({
    ...
    if ($_.Button -eq [System.Windows.Forms.MouseButtons]::Right) {
        ...
        $ArchetypeMenuStripToolDPIAware = [System.Drawing.Bitmap]::FromFile("$PWD\GUI Form Images\Icons\DPIAware.png")
        ...
        Do {
                ...
                if ($PokeMMOActive -ne $null) {
                    ...
                    if (($Script:CounterMode -match "Expanded_Encounter" -or $Script:CounterMode -match "Expanded_Fossil" -or $Script:CounterMode -match "Expanded_Egg" -or $Script:CounterMode -match "Collapsed_Encounter") -and ($GetPixelColor -match '\b('+($PixelMatchArray -join '|')+')\b')) {
                            ...
                            [IO.File]::AppendAllText("$PWD\Counter Functions\Core\DEBUG\DEBUG_OCR_OUTPUT.txt", "-----------------`n| OCR Language: |`n-----------------`n`n$SystemCurrentLangConfig`n`n################################`n#   END OF COUNTER OCR DEBUG   #`n################################")
